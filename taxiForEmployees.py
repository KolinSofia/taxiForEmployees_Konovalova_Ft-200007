...
try:
    ...
    print("Указанное количество сотрудников не является целым числом, обновите программу")
    msvcrt.getch()
    sys.exit()
# Проверка на вхождение employeesN в диапазон
if not ((employeesN>=1) and (employeesN<=1000)):
    print("Количество сотрудников не входит установленные рамки")
    msvcrt.getch()
    sys.exit()
# Ввод расстояний до домов сотрудников и проверка на целочисленность
try:
    employeesKM = list(map(int, input("Введите расстояния до дома сотрудника, в формате строки, из целых чисел от 1 до 999: ").split()))
except Exception:
...
print()
print("Соответствующие тарифы такси:", end=" ")
